...
class EventValidateTestCase(TestCase):
    ...
    def test_from_date_param_validate_raise_false(self):
        ...
        self.assertFalse(event_from_date_param_validate(valid_data))

    def test_event_paginator_raise_false_limit(self):
        valid_data = {'number': 2,
                      'limit111': 4}

        self.assertFalse(event_paginator_validate_limit(valid_data))

    def test_event_paginator_raise_error_number(self):
        valid_data = {'number11': 2,
                      'limit': 4}

        self.assertFalse(event_paginator_validate_number(valid_data))

    def test_event_paginator_validate_retfalse_invalid_number(self):
        invalid_data= {'number11': 2,
                      'limit': 4}
        self.assertFalse(event_paginator_validate(invalid_data))

    def test_event_paginator_validate_retfalse_invalid_limit(self):
        invalid_data= {'number': 2,
                      'limit111': 4}
        self.assertFalse(event_paginator_validate(invalid_data))

    def test_event_paginator_validate_retfalse_invalid_data(self):
        invalid_data= {'number111': 2,
                      'limit111': 4}
        self.assertTrue(event_paginator_validate(invalid_data))

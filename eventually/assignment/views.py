    ...
    def put(self, request, assignment_id=None):
        ...
        if status == STATUS_IS_DONE:
            if assignment.item.form == FORM_THEORETIC:
                ...
                assignment.update(**new_data)
                create_assigns_for_subordinate(assignment)
...
def get_assignment_list(request, topic_id, user_id=None):
    ...
    if request.method == "GET":
        if user_id:
            ...
            user = request.user
            assignments = Assignment.get_by_topic_item_ids(student_id=user, \
                                                           topic_id=topic_id)
        if assignments:
            data = {'assignments': [{'assignment': assignment.to_dict(), 'item': \
                assignment.item.to_dict()} for assignment in assignments]}
            return JsonResponse(data, status=200)
        return RESPONSE_404_OBJECT_NOT_FOUND
    ...
    for mentor_student in topic_students:
        ...
        if not superiors_ids:
            ...
            for superior_item_id in superiors_ids:
                superior_assignment = Assignment.get_by_topic_item_ids(
                    student_id=student_id, item_id=superior_item_id)
...
def create_assigns_for_subordinate(assignment):
    """

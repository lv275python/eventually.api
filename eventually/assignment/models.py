...
class Assignment(AbstractModel):
    ...
    @staticmethod
    def get_by_topic_item_ids(student_id, topic_id=None, item_id=None):
        """
    ...
    def get_curriculums(student_id):
        """
        Method that gets curriculums that belong to certain student
        :param student_id:  Certain student id
        :type student_id: int

        :return: QuerySet with curriculums
        """
    ...
    def get_topics(student_id, curriculum_id=None):
        """
        Method that gets topics that belong to certain student
        :param student_id: Certain student id
        :type student_id: int

        :param curriculum_id: certain curriculum id
        :type curriculum_id: int

        :return: QuerySet with topics
        """
        if curriculum_id:
            assignments = Assignment.objects.filter(user=student_id,
                                                    item__topic__curriculum=curriculum_id)\
                                                    .exclude(grade=True)
    ...
    def get_assignments_by_mentor_id(mentor_id, topic_id=None):
        """
        Method that assignments that belong to students of certain mentor
        :param mentor_id: Certain user-mentor id
        :type mentor_id: int
        ...
        """
        if topic_id:
            assignments = Assignment.objects.filter(user_id=mentor_id, item__topic_id=topic_id)\
                .exclude(grade=True)
        else:
            assignments = Assignment.objects.filter(user_id=mentor_id).exclude(grade=True)
        return assignments

    @staticmethod
    def get_curriculums_by_mentor_id(mentor_id):
        """
        Method that gets curriculums that belong to certain mentor-user

        :param mentor_id: certain mentor-user
        :type mentor_id: int

        :return: QuerySet with curriculums
        """
        assignments = Assignment.objects.filter(item__topic__mentors__in=[mentor_id])\
                .exclude(grade=True)
        curriculums_id = assignments.values_list('item__topic__curriculum', flat=True).distinct()
        curriculums = [Curriculum.get_by_id(id) for id in curriculums_id]
        return curriculums
    ...
    def get_topics_by_mentor_id(mentor_id, curriculum_id=None):
        """
        Method that gets topics that belong to certain mentor

        :param mentor_id: certain mentor-user id
        :type mentor_id: int
        ...
        if curriculum_id:
            ...
            assignments = Assignment.objects.filter(user=mentor_id).exlude(grade=True)
            mentor_topic_ids = assignments.values_list('item__topic').distinct()
            topics = [Topic.get_by_id(id) for id in mentor_topic_ids]
        return topics

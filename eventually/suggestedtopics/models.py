...
class SuggestedTopics(AbstractModel):
    ...
    def to_dict(self):
        ...
        if self.interested_users:
            interested_users = [user.id for user in self.interested_users.all()]
        interested_users_name = []
        if self.interested_users:
            for user in self.interested_users.all():
                interested_users_name.append(user.first_name + ' ' + user.last_name)
        ...
        return {'id': self.id,
                ...
                'interested_users_name': interested_users_name,
    ...
    def update(self, name=None, description=None, interested_user=None, remove_interest=None):
        ...
        :param interested_user: List of users id interested in topic
        :type interested_user: list of int

        :param remove_interest: if user is in list of interested_users
        :type remove_interest: boolean
        ...
        if not remove_interest:
            self.add_interested_user(interested_user)
        elif remove_interest:
